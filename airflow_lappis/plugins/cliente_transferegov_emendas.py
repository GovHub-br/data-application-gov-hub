...
class ClienteTransfereGov(ClienteBase):
    ...
    def get_planos_trabalho_especial_by_plano_acao(
        self, id_plano_acao: int, limit: int = 1000, offset: int = 0
    ) -> Optional[list]:
        """
        Obter planos de trabalho especiais por ID do plano de ação com paginação.
        ...
        Returns:
            list: lista de planos de trabalho especiais ou None se falhar
        """

        endpoint = f"plano_trabalho_especial?id_plano_acao=eq.{id_plano_acao}"
        ...
        logging.info(
            f"[cliente_transfere_gov.py] Fetching planos de trabalho especiais for "
        ...
        if status == http.HTTPStatus.OK and isinstance(data, list):
            logging.info(
                f"[cliente_transfere_gov.py] Successfully fetched {len(data)} "
                f"planos de trabalho especiais for plano_acao {id_plano_acao}"
            )
            return data
        else:
            logging.warning(
                f"[cliente_transfere_gov.py] Failed to fetch planos de trabalho especiais for "
                f"plano_acao {id_plano_acao} with status: {status}"
            )
            return None

    def get_all_planos_trabalho_especial_by_plano_acao(
        self, id_plano_acao: int, page_size: int = 1000
    ) -> list:
        """
        Obter todos os planos de trabalho especiais de um plano de ação com paginação automática.
        ...
        while True:
            data = self.get_planos_trabalho_especial_by_plano_acao(
